...
#define MAX_NUM_REG                                                                                \
    0x16 //!< \brief defines the value for temporary data to read all readable registers

#define DELETE_BIT_15 0x7FFF //!< \brief Value used to delete everything except the first 15 bits
#define CHANGE_UINT_TO_INT_15                                                                      \
    0x8000                  //!< \brief Value used to change unsigned 16bit integer into signed
#define CHECK_BIT_14 0x4000 //!< \brief
#define GET_BIT_14_4 0x7FF0 //!< \brief

#define DELETE_7BITS                                                                               \
    0x01FF //!< \brief values used to calculate 9 bit signed integer sent by the sensor
#define CHANGE_UNIT_TO_INT_9                                                                       \
    0x0200 //!< \brief Value used to change unsigned 9bit integer into signed
#define CHECK_BIT_9 0x0100

#define POW_2_15                                                                                   \
    32768.0 //!< \brief values used to for final calculations of angle speed, revolutions, range and value
#define POW_2_7       128.0  //!< \brief
#define ANGLE_360_VAL 360.0f //!< \brief
#define ANGLE_180_VAL 180.0f
...
enum errorTypes {
    ...
    CRC_ERROR =
        0xFF //!< \brief CRC_ERROR = Cyclic Redundancy Check (CRC), which includes the STAT and RESP bits wrong
};
