...
class Tle5012b {
  public:
    ...
    struct safetyWord { //!< \brief Safety word bit setting
        ...
        uint8_t CRC_;   //!< \brief bits 7:0 Status ADC Test
        ...
        slaveNum responseSlave() {
            return (RESP == 0x7
                        ? TLE5012B_S3
                        : (RESP == 0xB ? TLE5012B_S2 : (RESP == 0xD ? TLE5012B_S1 : TLE5012B_S0)));
        }
    ...
    //!< \brief constructor for the Sensor taking in a bus
    Tle5012b(SPI_HandleTypeDef& spi, GPIO_TypeDef* port, uint16_t pin);
	 ...
	 * @return CRC error type
	 */
    errorTypes readFromSensor(uint16_t command, uint16_t& data, updTypes upd = UPD_low,
	 ...
	 * @return CRC error type
	 */
    errorTypes readMoreRegisters(uint16_t command, uint16_t data[], updTypes upd = UPD_low,
                                 safetyTypes safe = SAFE_high);
	 ...
	 * @return CRC error type
	 */
    errorTypes readActivationStatus(uint16_t& data, updTypes upd = UPD_low,
                                    safetyTypes safe = SAFE_high);
	 ...
	 * @return CRC error type
	 */
    errorTypes getAngleValue(float& angleValue, int16_t& rawAnglevalue, updTypes upd = UPD_low,
                             safetyTypes safe = SAFE_high);
	 ...
	 * @return CRC error type
	 */
    errorTypes getNumRevolutions(int16_t& numRev, updTypes upd = UPD_low,
                                 safetyTypes safe = SAFE_high);
	 ...
	 * @return CRC error type
	 */
    errorTypes getTemperature(double& temp, int16_t& rawTemp, updTypes upd = UPD_low,
                              safetyTypes safe = SAFE_high);
	 ...
	 * @return CRC error type
	 */
    errorTypes getAngleSpeed(double& angleSpeed, int16_t& rawSpeed, updTypes upd = UPD_low,
                             safetyTypes safe = SAFE_high);
    ...
    /*!
	 * Function set the sensors filter decimation mode.
	 * @param [in] filMode the decimation filter mode to set
	 * @return CRC error type
	 */
    errorTypes setFilterDecimation(Reg::filterMode_t filMode);

  protected:
    uint16_t _command[2]; //!< \brief  command write data [0] = command [1] = data to write
    uint16_t _received
        [MAX_REGISTER_MEM]; //!< \brief fetched data from sensor with last word = safety word
    uint16_t _registers
        [CRC_NUM_REGISTERS
         + 1]; //!< \brief keeps track of the values stored in the 8 _registers, for which the CRC is calculated
