...
const Reg::BitField_t Reg::bitFields[] = {
    ...
    {REG_ACCESS_R, REG_STAT, 0x10, 4, 0x00,
     0}, //!< 04 bits 4:4 SDSPU status digital signal processing unit
    ...
    {REG_ACCESS_RU, REG_STAT, 0x1000, 12, 0x00,
     0}, //!< 12 bits 12:12 NOGMRA no valid GMR Angle Value
    ...
    {REG_ACCESS_RW, REG_ACSTAT, 0x1, 0, 0x00,
     1}, //!< 15 bits 0:0 ASRST Activation of Hardware Reset
    {REG_ACCESS_RWU, REG_ACSTAT, 0x2, 1, 0x00, 1}, //!< 16 bits 1:1 ASWD Enable DSPU Watch dog
    {REG_ACCESS_RWU, REG_ACSTAT, 0x4, 2, 0x00,
     1}, //!< 17 bits 2:2 ASVR Enable Voltage regulator Check
    ...
    {REG_ACCESS_RWU, REG_ACSTAT, 0x20, 5, 0x00,
     1}, //!< 20 bits 5:5 ASOV Enable of DSPU Overflow Check
    {REG_ACCESS_RWU, REG_ACSTAT, 0x40, 6, 0x00,
     1}, //!< 21 bits 6:6 ASVECXY Activation of X,Y Out of Limit-Check
    {REG_ACCESS_RWU, REG_ACSTAT, 0x80, 7, 0x00,
     1}, //!< 22 bits 7:7 ASVEGMAG Activation of Magnitude Check
    {REG_ACCESS_RES, REG_ACSTAT, 0x100, 8, 0x00, 1}, //!< 23 bits 8:8 Reserved
    {REG_ACCESS_RWU, REG_ACSTAT, 0x200, 9, 0x00,
     1}, //!< 24 bits 9:9 ASADCT Enable ADC Test vector Check
    {REG_ACCESS_RWU, REG_ACSTAT, 0x400, 10, 0x00,
     1}, //!< 25 bits 10:10 ASFRST Activation of Firmware Reset
    {REG_ACCESS_RES, REG_ACSTAT, 0xF800, 11, 0x00, 1}, //!< 26 bits 15:11 Reserved

    {REG_ACCESS_RU, REG_AVAL, 0x7FFF, 0, 0x00,
     2}, //!< 27 bits 14:0 ANGVAL Calculated Angle Value (signed 15-bit)
    {REG_ACCESS_R, REG_AVAL, 0x8000, 15, 0x00, 2}, //!< 28 bits 15:15 RDAV Read Status, Angle Value

    {REG_ACCESS_RU, REG_ASPD, 0x7FFF, 0, 0x00,
     3}, //!< 29 bits 14:0 ANGSPD Signed value, where the sign bit [14] indicates the direction of the rotation
    {REG_ACCESS_R, REG_ASPD, 0x8000, 15, 0x00, 3}, //!< 30 bits 15:15 RDAS Read Status, Angle Speed

    {REG_ACCESS_RU, REG_AREV, 0xFF, 0, 0x00,
     4}, //!< 31 bits 8:0 REVOL Revolution counter. Increments for every full rotation in counter-clockwise direction
    {REG_ACCESS_RWU, REG_AREV, 0x7E00, 9, 0x00,
     4}, //!< 32 bits 14:9 FCNT Internal frame counter. Increments every update period
    {REG_ACCESS_R, REG_AREV, 0x8000, 15, 0x00, 4}, //!< 33 its 15:15 RDREV Read Status, Revolution

    {REG_ACCESS_RWU, REG_FSYNC, 0xFF, 0, 0x00,
     5}, //!< 34 bits 8:0 TEMPR Signed offset compensated temperature value
    {REG_ACCESS_RU, REG_FSYNC, 0xFE00, 9, 0x00,
     5}, //!< 35 bits 15:9 FSYNC Frame Synchronization Counter Value

    {REG_ACCESS_RW, REG_MOD_1, 0x3, 0, 0x00, 6}, //!< 36 bits 1:0 IIFMOD Incremental Interface Mode
    {REG_ACCESS_RW, REG_MOD_1, 0x4, 2, 0x00,
     6}, //!< 37 bits 2:2 DSPUHOLD if DSPU is on hold, no watch dog reset is performed by DSPU
    {REG_ACCESS_RES, REG_MOD_1, 0x8, 3, 0x00, 6}, //!< 38 bits 3:3 Reserved1
    {REG_ACCESS_RW, REG_MOD_1, 0x10, 4, 0x00,
     6}, //!< 39 bits 4:4 CLKSEL switch to external clock at start-up only
    ...
    {REG_ACCESS_RW, REG_SIL, 0x40, 6, 0x00,
     7}, //!< 44 bits 6:6 ADCTVEN Sensor elements are internally disconnected and test voltages are connected to ADCs
    {REG_ACCESS_RES, REG_SIL, 0x380, 7, 0x00, 7}, //!< 45 bits 9:7 Reserved1
    {REG_ACCESS_RW, REG_SIL, 0x400, 10, 0x00,
     7}, //!< 46 bits 10:10 FUSEREL Triggers reload of default values from laser fuses into configuration registers
    {REG_ACCESS_RES, REG_SIL, 0x3800, 11, 0x00, 7}, //!< 47 bits 13:11 Reserved2
    {REG_ACCESS_RW, REG_SIL, 0x4000, 14, 0x00,
     7}, //!< 48 bits 14:14 FILTINV the X- and Y-signals are inverted. The angle output is then shifted by 180°
    {REG_ACCESS_RW, REG_SIL, 0x8000, 15, 0x00,
     7}, //!< 49 bits 15:15 FILTPAR the raw X-signal is routed also to the raw Y-signal input of the filter so SIN and COS signal should be identical

    {REG_ACCESS_RW, REG_MOD_2, 0x3, 0, 0x00,
     8}, //!< 50 bits 1:0 AUTOCAL Automatic calibration of offset and amplitude synchronicity for applications with full-turn
    {REG_ACCESS_RW, REG_MOD_2, 0x4, 2, 0x00,
     8}, //!< 51 bits 2:2 PREDICT Prediction of angle value based on current angle speed
    {REG_ACCESS_RW, REG_MOD_2, 0x8, 3, 0x00,
     8}, //!< 52 bits 3:3 ANGDIR Inverts angle and angle speed values and revolution counter behavior
    {REG_ACCESS_RW, REG_MOD_2, 0x7FF0, 4, 0x00,
     8}, //!< 53 bits 14:4 ANGRANGE Changes the representation of the angle output by multiplying the output with a factor ANG_RANGE/128
    ...
    {REG_ACCESS_RW, REG_MOD_3, 0x4, 2, 0x00,
     9}, //!< 56 bits 2:2 SSCOD SSC-Interface Data Pin Output Mode
    {REG_ACCESS_RW, REG_MOD_3, 0x8, 3, 0x00,
     9}, //!< 57 bits 3:3 SPIKEF Filters voltage spikes on input pads (IFC, SCK and CSQ)
    {REG_ACCESS_RW, REG_MOD_3, 0xFFF0, 4, 0x00,
     9}, //!< 58 bits 15:4 ANG_BASE Sets the 0° angle position (12 bit value). Angle base is factory-calibrated to make the 0° direction parallel to the edge of the chip

    {REG_ACCESS_RES, REG_OFFX, 0xF, 0, 0x00, 10}, //!< 59 bits 3:0 Reserved1
    {REG_ACCESS_RW, REG_OFFX, 0xFFF0, 4, 0x00,
     10}, //!< 60 bits 15:4 XOFFSET 12-bit signed integer value of raw X-signal offset correction at 25°C

    {REG_ACCESS_RES, REG_OFFY, 0xF, 0, 0x00, 11}, //!< 61 bits 3:0 Reserved1
    {REG_ACCESS_RW, REG_OFFY, 0xFFF0, 4, 0x00,
     11}, //!< 62 bits 15:4 YOFFSET 12-bit signed integer value of raw Y-signal offset correction at 25°C

    {REG_ACCESS_RES, REG_SYNCH, 0xF, 0, 0x00, 12}, //!< 63 bits 3:0 Reserved1
    {REG_ACCESS_RW, REG_SYNCH, 0xFFF0, 4, 0x00,
     12}, //!< 64 bits 15:4 SYNCH 12-bit signed integer value of amplitude synchronicity

    {REG_ACCESS_RW, REG_IFAB, 0x3, 0, 0x00,
     13}, //!< 65 bits 1:0 IFADHYST Hysteresis (multi-purpose)
    {REG_ACCESS_RW, REG_IFAB, 0x4, 2, 0x00, 13}, //!< 66 bits 2:2 IFABOD IFA,IFB,IFC Output Mode
    {REG_ACCESS_RW, REG_IFAB, 0x8, 3, 0x00,
     13}, //!< 67 bits 3:3 FIRUDR Initial filter update rate (FIR)
    {REG_ACCESS_RW, REG_IFAB, 0xFFF0, 4, 0x00,
     13}, //!< 68 bits 15:4 ORTHO Orthogonality Correction of X and Y Components

    {REG_ACCESS_RW, REG_MOD_4, 0x3, 0, 0x00,
     14}, //!< 69 bits 1:0 IFMD Interface Mode on IFA,IFB,IFC
    {REG_ACCESS_RES, REG_MOD_4, 0x4, 2, 0x00, 14}, //!< 70 bits 2:2 Reserved1
    {REG_ACCESS_RW, REG_MOD_4, 0x18, 3, 0x00,
     14}, //!< 71 bits 4:3 IFABRES IIF resolution (multi-purpose)
    {REG_ACCESS_RW, REG_MOD_4, 0x1E0, 5, 0x00,
     14}, //!< 72 bits 8:5 HSMPLP Hall Switch mode (multi-purpose)
    {REG_ACCESS_RW, REG_MOD_4, 0x7E00, 9, 0x00,
     14}, //!< 73 bits 15:9 TCOXT 7-bit signed integer value of X-offset temperature coefficient
    ...
    {REG_ACCESS_RW, REG_TCO_Y, 0x7E00, 9, 0x00,
     15}, //!< 76 bits 15:9 TCOYT 7-bit signed integer value of Y-offset temperature coefficient
    ...
    {REG_ACCESS_RU, REG_D_MAG, 0x3FF, 0, 0x00,
     18}, //!< 79 bits 9:0 MAG Unsigned Angle Vector Magnitude after X, Y error compensation (due to temperature)
    {REG_ACCESS_RES, REG_D_MAG, 0xFC00, 10, 0x00, 18}, //!< 80 bits 15:10 Reserved1

    {REG_ACCESS_RU, REG_T_RAW, 0x3FF, 0, 0x00,
     19}, //!< 81 bits 9:0 TRAW Temperature Sensor Raw-Value at ADC without offset
    {REG_ACCESS_RES, REG_T_RAW, 0xFC00, 10, 0x00, 19}, //!< 82 bits 14:10 Reserved1
    {REG_ACCESS_RU, REG_T_RAW, 0x8000, 15, 0x00,
     19}, //!< 83 bits 15:15 TTGL Temperature Sensor Raw-Value Toggle toggles after every new temperature value

    {REG_ACCESS_RU, REG_IIF_CNT, 0x7FFF, 0, 0x00,
     20}, //!< 84 bits 14:0 IIFCNT 14 bit counter value of IIF increments
    {REG_ACCESS_RES, REG_IIF_CNT, 0x8000, 15, 0x00, 20}, //!< 85 bits 15:14 Reserved1

    {REG_ACCESS_R, REG_T25O, 0x1FFF, 0, 0x00,
     21}, //!< 86 bit 8:0 T250 Signed offset value at 25°C temperature; 1dig=0.36°C
...
bool Reg::getBitField(BitField_t bitField, uint16_t& bitFValue) {
    ...
    if ((REG_ACCESS_R & bitField.regAccess) == REG_ACCESS_R) {
        ...
        p->readFromSensor(addrFields[bitField.posMap].regAddress, regMap[bitField.posMap], UPD_low,
                          SAFE_high);
...
bool Reg::setBitField(BitField_t bitField, uint16_t bitFNewValue) {
    ...
    if ((REG_ACCESS_W & bitField.regAccess) == REG_ACCESS_W) {
        Tle5012b* p = static_cast<Tle5012b*>(parent_);
        regMap[bitField.posMap] = (regMap[bitField.posMap] & ~bitField.mask)
                                  | ((bitFNewValue << bitField.position) & bitField.mask);
