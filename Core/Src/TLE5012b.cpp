 ...
 */
double calculateAngleSpeed(double angRange, int16_t rawAngleSpeed, uint16_t firMD,
    ...
    finalAngleSpeed = ((angRange / POW_2_15) * ((double)rawAngleSpeed))
                      / (((double)predictionVal) * firMDVal * microsecToSec);
...
Tle5012b::Tle5012b(SPI_HandleTypeDef& spi, GPIO_TypeDef* port, uint16_t pin) : reg(this) {
    sBus = new SPIC(spi, port, pin);
    en = NULL;
    safetyWord = 0;
    mSlave = TLE5012B_S0;
}
...
errorTypes Tle5012b::readFromSensor(uint16_t command, uint16_t& data, updTypes upd,
                                    safetyTypes safe) {
...
errorTypes Tle5012b::readMoreRegisters(uint16_t command, uint16_t data[], updTypes upd,
                                       safetyTypes safe) {
    errorTypes checkError = NO_ERROR;
...
//-----------------------------------------------------------------------------
// begin CRC functions
errorTypes Tle5012b::checkSafety(uint16_t safety, uint16_t command, uint16_t* readreg,
                                 uint16_t length) {
...
errorTypes Tle5012b::readBlockCRC() {
    ...
    errorTypes checkError =
        checkSafety(_registers[8], READ_BLOCK_CRC, _registers, CRC_NUM_REGISTERS);
    resetSafety();
    return (checkError);
}
...
errorTypes Tle5012b::getAngleValue(float& angleValue, int16_t& rawAnglevalue, updTypes upd,
                                   safetyTypes safe) {
    uint16_t rawData = 0;
    ...
    angleValue = (ANGLE_360_VAL / POW_2_15) * ((float)rawAnglevalue);
    return (status);
}
...
errorTypes Tle5012b::getTemperature(double& temperature, int16_t& rawTemp, updTypes upd,
                                    safetyTypes safe) {
    uint16_t rawData = 0;
...
errorTypes Tle5012b::getAngleSpeed(double& finalAngleSpeed, int16_t& rawSpeed, updTypes upd,
                                   safetyTypes safe) {
...
errorTypes Tle5012b::setFilterDecimation(Reg::filterMode_t filMode) {
    uint16_t rawData = 0;

    errorTypes status = readIntMode1(rawData);
    if (status != NO_ERROR) {
        return (status);
    }
    ...
    rawData &= ~(1UL << 14);
    rawData &= ~(1UL << 15);
    rawData = rawData | filMode;
    status = writeIntMode1(rawData);
